...
class CadastroPF {
    ...
    cadastroConcluido() {
        ...
        cy.get('#mat-input-4').invoke('val').then((cpf) => {
            if (!cpf) {
                throw new Error('Campo CPF não preenchido. Cadastro não pode ser concluído.');
            }
        });

        cy.get('#mat-input-16').invoke('val').then((nome) => {
            if (!nome) {
                throw new Error('Campo Nome não preenchido. Cadastro não pode ser concluído.');
            }
        });

        cy.get('#mat-input-27').invoke('val').then((ddd) => {
            if (!ddd) {
                throw new Error('Campo DDD não preenchido. Cadastro não pode ser concluído.');
            }
        });
    
        cy.get('#mat-input-28').invoke('val').then((telefone) => {
            if (!telefone) {
                throw new Error('Campo Telefone não preenchido. Cadastro não pode ser concluído.');
            }
        });
