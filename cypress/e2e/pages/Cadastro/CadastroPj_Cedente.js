...
class CadastroPJCedente {
    ...
    preencherCadastroPjCedente(){
        ...
        const cnpjcedente = cnpj;
        cy.get('#mat-input-4').type(cnpj);
        cy.writeFile('cypress/fixtures/cpf/cnpj_cedente.json',{cnpjcedente});
        cy.get('#btn-salvar').click();
        cy.get('#mat-input-16').type('Lucas Silva e Silva');
        cy.get('body').type('{esc}');
        cy.get('.z-index-8 > .ng-invalid > .w-select > .w-select-input').click();
        cy.get('.ng-invalid > .w-select > .overlay > .w-select-list > [label="Selecionar todos"]').click();
        cy.get('body').type('{esc}');
        cy.get('.z-index-7 > wba-select.ng-untouched > .w-select > .w-select-input').click();
        cy.get('.z-index-7 > wba-select.ng-pristine > .w-select > .overlay > .w-select-list > [label="Selecionar todos"]').click();
        cy.get('body').type('{esc}');
        cy.get('.ng-invalid > .w-select > .w-select-input').click();
        cy.get('.ng-invalid > .w-select > .overlay > .w-select-list > [ng-reflect-label="FIDC - RENAN FIDC SAAA"] > .label-option').click();
        cy.get('body').type('{esc}');
       ...
       //cy.contains('Risco AA').click();
        ...
        cy.get('#input-Fator').type('11');
        cy.get('#input-Valorem').type('1');
        cy.get('.w-select-input').click();
        cy.contains('Nominal').click();
